...
fn eval_map_energization(map: &Vec<Vec<(TileState, TileType)>>) -> i32 {
...
pub fn process(_input: &str) -> miette::Result<i32> {
    ...
    let map = parse_map(_input);
    print_map(&map, MapPrintMode::Types);

    let mut energization_values = Vec::<i32>::new();
    for x in 0..width as i32 {
        let mut map_cpy = map.clone();
        energize_map(&mut map_cpy, (x, 0, Direction::Down), &mut Vec::new());
        energization_values.push(eval_map_energization(&map_cpy));
    }
    for x in 0..width as i32 {
        let mut map_cpy = map.clone();
        energize_map(
            &mut map_cpy,
            (x, height - 1, Direction::Up),
            &mut Vec::new(),
        );
        energization_values.push(eval_map_energization(&map_cpy));
    }
    for y in 0..height as i32 {
        let mut map_cpy = map.clone();
        energize_map(&mut map_cpy, (0, y, Direction::Right), &mut Vec::new());
        energization_values.push(eval_map_energization(&map_cpy));
    }
    for y in 0..height as i32 {
        let mut map_cpy = map.clone();
        energize_map(
            &mut map_cpy,
            (width - 1, y, Direction::Left),
            &mut Vec::new(),
        );
        energization_values.push(eval_map_energization(&map_cpy));
    }

    let max_energization = energization_values.iter().max().unwrap();
    Ok(*max_energization)
}
